...
export function useUserMessages(
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`my_inbox_${userId}`)
      .on(
        'postgres_changes',
  ...
  const { mutateAsync: markAsRead } = useMutation({
    ...
    onError: (error: Error) => { // error 타입을 명시
      ...
      // --- 토스트 추가 ---
      toast.error(`읽음 처리 실패: ${error.message}`); // ✅ 3. 실패 토스트 추가
      // ------------------
    },
  });
  ...
  return { 
    ...
    isLoading, 
    error: error as string | null, // 타입 캐스팅
    markAsRead, 
    refetch: () => queryClient.invalidateQueries({ queryKey: queryKey }),
    sendMessage: (peerId: string, title: string, text: string) => 
      sendMessage({ peerId, title, text }),
  };
}