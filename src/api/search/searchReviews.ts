 ...
 * @returns {Promise<ReviewWithDetail[]>} - 검색된 리뷰 목록 (좋아요, 댓글 포함)
 */
export const searchReviews = async (
  query: string,
): Promise<ReviewWithDetail[]> => {
  ...
  try {
    ...
    // reviews는 SupabaseRawResult[] 같은 타입이므로,
    // 우리가 정의한 ReviewWithDetail[] 타입으로 매핑합니다.
    const finalData: ReviewWithDetail[] = reviews.map((review) => ({
      ...review,
      author_name: review.users?.[0]?.name,
      comments: review.comments?.[0]?.count ?? 0,
