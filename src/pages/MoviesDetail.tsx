    ...
    <div className="w-3/4 p-4 overflow-y-auto">
      ...
      <div className="flex gap-8">
        ...
        <div className="w-[250px] flex-shrink-0">
          {isLoading || !movie ? (
            <Skeleton height={358} className="rounded-lg" baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
          ) : (
            <img src={movie.posterUrl} alt={movie.title} className="w-[250px] h-[358px] object-cover rounded-lg card-shadow" />
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          <h1 className="text-5xl font-extrabold text-[var(--color-text-main)]">
            {isLoading || !movie ? <Skeleton width={280} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} /> : movie.title}
          </h1>

          {/* CTA 버튼 영역 */}
          <div className="flex items-center gap-4 mt-1">
            <button type="button" className="h-12 px-6 rounded-[14px] bg-gradient-to-r from-[#8b5cf6] to-[#a78bfa] text-white font-semibold shadow-md hover:opacity-90 active:scale-[0.99] transition">
          ...
          <div className="flex flex-col gap-2">
            {isLoading || !movie ? (
              <>
                <Skeleton width={220} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
                <Skeleton width={260} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
                <Skeleton count={2} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
              </>
            ) : (
              <>
          ...
          <div className="mt-2">
            {isLoading || !movie ? (
              <Skeleton count={4} baseColor={skeletonBaseColor} highlightColor={skeletonHighlightColor} />
            ) : (
